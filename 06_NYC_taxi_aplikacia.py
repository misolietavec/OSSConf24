...
def _(day_choose, drop_plot, hour_choose, mo, pick_plot):
    tab_map = mo.vstack(
        [
            mo.hstack([day_choose, hour_choose], justify="start"),
            mo.hstack([pick_plot(day_choose.value, hour_choose.value), drop_plot(day_choose.value, hour_choose.value)]),
        ]
    )
    return tab_map,
...
def _(histo_dists, histo_times, mo, nbins_choose, quant_choose):
    tab_histo = mo.vstack(
        [
            mo.hstack([nbins_choose, quant_choose], justify="start"),
...
def _(mo, tab_daily, tab_histo, tab_map, tab_monthly):
    tabs = mo.ui.tabs({"Grafy mesačné": tab_monthly, "Miesta na mape": tab_map, "Grafy po dňoch": tab_daily, "Histogramy": tab_histo})
    return tabs,
