...
@app.cell
def __(pl):
    # nacitame frejmu, stlpce pre vzdialenosti a casy jazd
    cols = ["distance", "rtime"]
...
def __(df_dt, np):
    ...
    qd = df_dt["distance"].quantile(0.97)
    qt = df_dt["rtime"].quantile(0.97)
...
def __(df_dt, np, pl, px):
    ...
    def plot_histo(col, rozsah, nbins=20):
        ...
        return px.bar(
            ...
            y="y",
            barmode="group",
            labels={"x": xlabel, "y": "Početnosť", "variable": "hodnota"},
            width=900,
            height=350,
        )
...
@app.cell
def __(plot_histo):
    plot_histo("rtime", (0, 40), 40)  # skusat rozne nbins aj hornu hranicu
    return
